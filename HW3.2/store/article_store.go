...
import (
	...
	"github.com/LightAlykard/GB-observability-HW/HW3/e"
...
func (e ErrNotFound) Error() string {
	return fmt.Sprintf("article with id %s not found", e.Id)
}
...
func (s ArticleStore) Add(c context.Context, article m.Article) error {
	...
	span.LogFields(
		log.String("store article add +", fmt.Sprintf("%v", article)),
	)
	return nil
}
...
func (s ArticleStore) Search(c context.Context, query string) ([]m.Article, error) {
	...
	for _, hit := range hits {
		...
		if err != nil {
			s.logger.Error(fmt.Sprintf(`cant decode struct: %s`, err))
			span.LogFields(
				log.Error(err),
			)
			return nil, err
