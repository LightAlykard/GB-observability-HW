...
func (e E) Insert(c context.Context, i I) error {
	...
	if err != nil {
		e.logger.Error(fmt.Sprintf(`cant marshal: %s`, err))
		span.LogFields(
			log.Error(err),
		)
		return err
...
func (e E) Search(c context.Context, q string) (SearchResponse, error) {
	...
	if err := json.NewDecoder(res.Body).Decode(&r); err != nil {
		e.logger.Error(fmt.Sprintf(`cant unmarshal: %s`, err))
		span.LogFields(
			log.Error(err),
		)
		return r, err
...
func (e E) Get(c context.Context, id string) (M, error) {
	...
	span.LogFields(
		log.String("ID ElasticSearch +", id),
	)
	return r, nil
}
